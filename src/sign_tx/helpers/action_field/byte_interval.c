...
uint16_t end_index(byte_interval_t *interval) {
    return interval->start_index_in_tx + interval->byte_count;
}
...
void print_interval(byte_interval_t *interval) {
    PRINTF("[%d-%d] (#%d bytes)", interval->start_index_in_tx, end_index(interval), interval->byte_count);
}

void zero_out_interval(byte_interval_t *interval) {
    interval->byte_count = 0;
    interval->start_index_in_tx = 0;
}